...
Vector3f RayColor(const Ray& ray, Scene& scene, int depth=0, bool test=false)
{
	...
	float R = 0.1f;
	float delta = 2 * 3.1415926 / t;
	float dr = R / t;
	for (int i = 0; i < t; i++)
	{
		for (int j = 0; j < t; j++) {
			float x = R * cos(delta * i) * sin(delta * j) * 2;
			float y = R * sin(delta * i) * sin(delta * j) * 2;
			float z = R * cos(delta * j) * 2;
			Vector3f lightpos = center + Vector3f(x, y, z);
			lightposes.push_back(lightpos);
		}
	}

	int n = lightposes.size();
	int cnt = 0;
	if (closest_hp.objIdx != -1)
	{
		...
		for (int j = 0; j < n; j++)
		{
			Ray shadow_ray(closest_hp.position, lightposes[j] - closest_hp.position);

			//请在以下部分加入对shadow ray是否被场景遮挡的判断，并对isShadow这个变量进行修改 
			for (int i = 0; i < scene.ObjectCount(); ++i)
			{
				HitInfo hit;
				bool bhit = scene.GetObjectPtr(i)->Hit(shadow_ray, hit);
				if (bhit && hit.t > 0 && hit.t < closest_hp.t)
				{
					isShadow = true;
					break;
				}
			...
			{
				//漫反射
				Vector3f lightdir = (lightposes[j] - closest_hp.position).normalized();
