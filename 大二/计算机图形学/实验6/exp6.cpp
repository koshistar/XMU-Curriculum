...
void make_frame(float h, int t)
{
	...
	for (int j = height - 1; j >= 0; --j)
	{
		for (int i = 0; i < width; ++i)
		{
			...
			else if (t == 1)
			{
				...
				outrlt.SetPixel(j, i, int(k * 255.99 * color[0] + (1 - k) * r), int(k * 255.99 * color[1] + (1 + k) * g), int(k * 255.99 * color[2] + (1 - k) * b));
			}
			else
			{
				unsigned r1, g1, b1;
				unsigned r2, g2, b2;
				float k1 = 0.7, k2 = 0.1, k3 = 0.2;
				pre_ppm.GetPixel(j, i, r1, g1, b1);
				pre_pre_ppm.GetPixel(j, i, r2, g2, b2);
				outrlt.SetPixel(j, i, int(k1 * 255.99 * color[0] + k2 * r1 + k3 * r2), int(k1 * 255.99 * color[1] + k2 * g1 + k3 * g2), int(k1 * 255.99 * color[2] + k2 * b1 + k3 * b2));
			}
		}
	}
	pre_pre_ppm = pre_ppm;
	...
	char path[50];
	sprintf_s(path, "./Frame//frame%d.ppm", t);
...
int main()
{
	system("rd /s /q Frame");
	system("md Frame");
	int n = 150, cnt = 10, t = 0;// 帧数、最大弹跳次数、目前弹跳次数
	float h = 4.0f, g = -0.0098f, v = 0, x = 0, k = 0.6;//高度、重力加速度、速度、位移、弹性系数
	for (int i = 0; i < n; i++)
	{
		if (t <= cnt)
		{
			x += v + 0.5 * g;//t=1匀加速直线运动
			v += g;
			if (x + h < 0.5f)
			{
				v = -k * v;
	...
	cout << "Start generate gif\n";
	vector<Image> frames;
	for (int i = 0; i < n; i++)
	{
		char path[50];
		sprintf_s(path, "Frame//frame%d.ppm", i);
		Image image;
		image.read(path);
