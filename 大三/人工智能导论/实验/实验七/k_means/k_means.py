...
class KMeans:
    ...
    def train(self,max_iterations):
        ...
        for _ in range(max_iterations):
            ...
            #4.进行中心点位置更新
            new_centroids = KMeans.centroids_compute(self.data,closest_centroids_ids,self.num_clusters)
            # 空簇处理：若某簇无样本点，从数据中随机重新选点
            for k in range(self.num_clusters):
                mask = (closest_centroids_ids == k).flatten()
                if mask.sum() == 0:
                    rand_idx = np.random.randint(0, num_examples)
                    new_centroids[k] = self.data[rand_idx]
    ...
    def centroids_compute(data,closest_centroids_ids,num_clusters):
        ...
        for centroid_id in range(num_clusters):
            ...
            members = data[closest_ids.flatten(),:]
            if members.shape[0] > 0:
                centroids[centroid_id] = np.mean(members, axis=0)
            # 若某簇无样本点，保留零向量（调用方会处理）
        return centroids
